...
import { ArrowLeft, Package, Truck, CheckCircle, AlertCircle, Tag } from "lucide-react"
...
export default function OrderDetailsPage() {
  ...
  // Função para renderizar o progresso do pedido
      ...
      <div className="w-full py-4">
        <div className="flex items-center justify-between">
          {steps.map((step, index) => (
            <div key={step.id} className="flex flex-col items-center">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full ${index <= currentStepIndex ? "bg-amber-500 text-white" : "bg-gray-200 text-gray-400"
                  }`}
              >
                {index + 1}
              </div>
        ...
        <div className="mt-4 h-1 w-full bg-gray-200">
          <div
            ...
            style={{
              width: `${currentStepIndex >= 0 ? Math.min(100, ((currentStepIndex + 0.5) / (steps.length - 1)) * 100) : 0
                }%`,
            }}
          />
        </div>
              ...
              <div className="rounded-lg bg-white p-6 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600">Realizado em {formatDate(order.transaction.createdAt)}</p>
                  </div>
                    ...
                    return Object.values(groupedItems).map((group: any) => (
                      <div key={group.bookDetails.id} className="flex border-b pb-4 last:border-0 last:pb-0">
                        <div className="relative h-24 w-16 flex-shrink-0 overflow-hidden rounded-md bg-gray-100">
                          <Image
                            src={`/placeholder.svg?height=300&width=200&text=${encodeURIComponent(
              ...
              <div className="rounded-lg bg-white p-6 shadow-sm">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span>R$ {order.transaction.amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {order.transaction.coupon && (
                    <div className="flex justify-between text-green-600">
                      <span>Cupom {order.transaction.coupon.code}</span>
                      <span>
                        -
                        {order.transaction.coupon.type === "PERCENTAGE"
                          ? `${order.transaction.coupon.discount}%`
                          : `R$${order.transaction.coupon.discount.toFixed(2)}`}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between font-bold">
                    <span>Total</span>
                    <span>R$ {order.transaction.getFinalPrice().toFixed(2)}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {order.transaction.card && (
                    <div className="flex items-center gap-2">
                      <div className="rounded-md bg-gray-100 p-2">
                        <Image
                          src={`/placeholder.svg?height=30&width=40&text=${brands.find((brand) => brand.id == order.transaction.card?.brandId)?.imageUrl}`}
                          alt={
                            brands.find((brand) => brand.id == order.transaction.card?.brandId)?.name ?? 'Cartão'
                          }
                          width={40}
                          height={30}
                        />
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {order.transaction.card.getBrandName()} terminado em{" "}
                          {order.transaction.card.getLastFourDigits()}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {order.transaction.coupon && (
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <div>
                        <p className="font-medium">Cupom de desconto</p>
                        <p className="text-sm text-green-600">
                          {order.transaction.coupon.code} (
                          {order.transaction.coupon.type === "PERCENTAGE"
                            ? `${order.transaction.coupon.discount}% de desconto`
                            : `R$${order.transaction.coupon.discount.toFixed(2)} de desconto`}
                          )
                        </p>
                      </div>
                    </div>
