...
export class Order {
  ...
  constructor(items: StockBook[], customer: Customer, address: Address, status = "PROCESSING") {
    this.items = items
  ...
  private validateStatusTransition(currentStatus: OrderStatus, newStatus: OrderStatus): void {
    ...
    if (!validTransitions[currentStatus].includes(newStatus)) {
      throw new EntityValidationException(`Transição de status inválida de ${currentStatus} para ${newStatus}`)
    }
  }
}
