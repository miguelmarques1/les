    ...
    return raw.map(
      (row) =>
        new MonthlySalesOutputDTO(
          row.month,
          Number.parseInt(row.monthNumber),
          Number.parseFloat(row.totalSales),
          Number.parseInt(row.totalOrders),
          Number.parseFloat(row.averageOrderValue),
          Number.parseInt(row.year),
        ),
    )
  },
