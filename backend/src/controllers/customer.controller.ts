...
import { CreateCustomerInputDTO, DeleteCustomerInputDTO, UpdateCustomerInputDTO } from "../dto/customer.dto";
...
export class CustomerController extends BaseController {
  ...
  async delete(req: Request, res: Response) {
    try {
      const id = parseInt(req['cus_id']);
      const input: DeleteCustomerInputDTO = req.body;
      input.customerId = id;

      const output = await this.customerService.delete(input);
